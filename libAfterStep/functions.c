...
ComplexFunction *find_complex_func (struct ASHashTable * list, char *name)
{
	ASHashData hdata = { 0 };
	if (name && list)
		if (get_hash_item (list, AS_HASHABLE (name), &hdata.vptr) !=
				ASH_Success)
...
{
#ifdef DEBUG_ALLOCS
	LOCAL_DEBUG_CALLER_OUT ("menu_data_item_destroy(\"%s\",%p)",
													((mdi
														&& mdi->fdata) ? (mdi->fdata->name) : "NULL"),
													mdi);
#endif
	if (mdi) {
...
#ifdef DEBUG_ALLOCS
			LOCAL_DEBUG_CALLER_OUT ("menu_data_destroy(\"%s\", %p)",
															md->name ? md->name : "(null)", md);
#endif
			if (md->name)
				free (md->name);
			if (md->comment)
...
void assign_minipixmaps (MenuDataItem * mdi, MinipixmapData * minipixmaps)
{
	if (mdi && minipixmaps) {
		...
		for (i = 0; i < MINIPIXMAP_TypesNum; ++i) {
			LOCAL_DEBUG_OUT ("type = %d, filename = \"%s\"", i,
											 minipixmaps[i].filename);
...
Bool check_fdata_availability (FunctionData * fdata)
{
	...
	if (fdata->func < F_ExecToolStart || fdata->func > F_ExecToolEnd) {
		if (IsSwallowFunc (fdata->func) || IsExecFunc (fdata->func)) {
			LOCAL_DEBUG_OUT ("now really checking availability for \"%s\"",
											 fdata->name ? fdata->name : "nameless");
			if (!is_executable_in_path (fdata->text)) {
				LOCAL_DEBUG_OUT ("unavailable :  \"%s\"",
												 fdata->name ? fdata->name : "nameless");
				return False;
			}
		} else if (fdata->func == F_CHANGE_BACKGROUND_FOREIGN
							 && !is_web_background (fdata)) {
			ASImageFileTypes type = check_asimage_file_type (fdata->text);

			LOCAL_DEBUG_OUT ("foreign back image \"%s\" has type %d",
											 fdata->text, type);
			if (type > ASIT_Supported) {
				LOCAL_DEBUG_OUT ("foreign back image of unknown type :  \"%s\"",
												 fdata->text);
				return False;
			}
		}
		...
		if (!is_executable_in_path (target)) {
			LOCAL_DEBUG_OUT ("unavailable :  \"%s\", target = \"%s\"",
											 fdata->name ? fdata->name : "nameless", target);
			return False;
		}
	}
...
static void check_availability (MenuDataItem * mdi)
{
	clear_flags (mdi->flags, MD_Disabled);
#ifndef NO_AVAILABILITYCHECK
	LOCAL_DEBUG_OUT ("checking availability for \"%s\"",
									 mdi->fdata->name ? mdi->fdata->name : "nameless");
...
MenuDataItem *add_menu_fdata_item (MenuData * menu, FunctionData * fdata,
																	 MinipixmapData * minipixmaps)
{
	MenuDataItem *mdi = NULL;
...
ASImage *check_scale_menu_pmap (ASImage * im, ASFlagType flags)
{
	if (im) {
		int w = im->width;
		int h = im->height;
		...
		if (w != im->width || h != im->height) {
			return scale_asimage (ASDefaultVisual, im, w, h, ASA_ASImage, 100,
...
void free_menu_pmaps (MenuData * menu)
{
	...
	LOCAL_DEBUG_OUT ("menu = %p, image_manager = %p", menu,
...
load_menuitem_pmap (MenuDataItem * mdi, MinipixmapTypes type, Bool force)
{
	...
	if (filename == NULL) {
		if (type == MINIPIXMAP_Icon) {
			if (func == F_CHANGE_BACKGROUND_FOREIGN
	...
	LOCAL_DEBUG_OUT ("type = %d, filename = \"%s\", fdata->text = \"%s\"",
									 type, filename, mdi->fdata->text);
	if (filename) {
		...
		LOCAL_DEBUG_OUT ("minipixmap target height = %d for \"%s\"", h,
										 minipixmap->filename);
		...
		if (func == F_CHANGE_BACKGROUND || func == F_CHANGE_BACKGROUND_FOREIGN) {
			...
			minipixmap->image = load_environment_icon_any (filename, h);
		}
...
reload_menuitem_pmap (MenuDataItem * mdi, MinipixmapTypes type, Bool force)
{
	...
	LOCAL_DEBUG_OUT ("type = %d, fdata->text = \"%s\"", type,
									 mdi->fdata->text);

	if (minipixmap->image) {
		if (minipixmap->filename != NULL && !force) {
			if (get_asimage_file_type
					(ASDefaultScr->image_manager,
					 minipixmap->filename) != ASIT_XMLScript) {
				ASImage *im = minipixmap->image;

				if (im->imageman != NULL
						&& im->imageman != ASDefaultScr->image_manager) {
...
MenuDataItem *menu_data_item_from_func (MenuData * menu,
																				FunctionData * fdata,
																				Bool do_check_availability)
{
	...
	if (fdata != NULL) {
		...
		if (mtype < MINIPIXMAP_TypesNum) {
			...
			set_string (&(item->minipixmap[mtype].filename),
									mystrdup (fdata->name));
		} else {
			item = new_menu_data_item (menu);
...
void
print_func_data (const char *file, const char *func, int line,
								 FunctionData * data)
{
	fprintf (stderr, "%s:%s:%s:%d>!!FUNC ", get_application_name (), file,
					 func, line);
	if (data == NULL)
		...
		if (term == NULL)
			...
			fprintf (stderr, "%s \"%s\" text[%s] ", term->keyword,
							 data->name ? data->name : "", data->text ? data->text : "");
			fprintf (stderr, "val0[%ld%c(%ld)] ", (long)data->func_val[0],
							 (data->unit[0] == '\0') ? ' ' : data->unit[0],
							 (long)data->unit_val[0]);
			fprintf (stderr, "val1[%ld%c(%ld)] ", (long)data->func_val[1],
							 (data->unit[1] == '\0') ? ' ' : data->unit[1],
							 (long)data->unit_val[1]);
