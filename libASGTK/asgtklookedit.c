...
static void asgtk_mystyle_edit_style_set (GtkWidget * widget,
																					GtkStyle * prev_style);
...
GType asgtk_mystyle_edit_get_type (void)
{
	...
	if (!id_type) {
		...
		id_type =
				g_type_register_static (GTK_TYPE_VBOX, "ASGtkMyStyleEdit",
																&id_info, 0);
	}
...
static void
asgtk_mystyle_edit_style_set (GtkWidget * widget, GtkStyle * prev_style)
{
	/* ASGtkImageDir *id = ASGTK_IMAGE_DIR (widget); */

	GTK_WIDGET_CLASS (mystyle_edit_parent_class)->style_set (widget,
																													 prev_style);
}
...
void
asgtk_mystyle_edit_set_line_enabled (ASGtkMyStyleEdit * self,
																		 ASGtkMyStyleOptions opt, Bool enabled)
{
	...
	switch (opt) {
	case ASGtkMSO_Name:
		container = GTK_CONTAINER (self->hbox1_mystyle_name);
		break;
	case ASGtkMSO_Overlay:
		toggle = GTK_TOGGLE_BUTTON (self->tgl2_overlay);
		container = GTK_CONTAINER (self->hbox2_overlay);
		break;
	case ASGtkMSO_Inherits:
		toggle = GTK_TOGGLE_BUTTON (self->tgl3_inherit);
		container = GTK_CONTAINER (self->hbox3_inherit);
		break;
	case ASGtkMSO_Font:
		toggle = GTK_TOGGLE_BUTTON (self->tgl4_font);
		container = GTK_CONTAINER (self->hbox4_font);
		break;
	case ASGtkMSO_Colors:
		toggle = GTK_TOGGLE_BUTTON (self->tgl5_colors);
		container = GTK_CONTAINER (self->hbox5_colors);
		break;
	case ASGtkMSO_Shadow:
		toggle = GTK_TOGGLE_BUTTON (self->tgl5_shadow);
		container = GTK_CONTAINER (self->hbox6_shadow);
		break;
	case ASGtkMSO_Background:
		toggle = GTK_TOGGLE_BUTTON (self->tgl7_background);
		container = GTK_CONTAINER (self->hbox7_background);
		break;
	...
	if (toggle) {
		...
		gtk_button_set_image (GTK_BUTTON (toggle),
													gtk_image_new_from_stock (enabled ? GTK_STOCK_YES
																										: GTK_STOCK_NO,
	...
	if (container)
		gtk_container_forall (container,
													enabled ? set_container_child_sensitive :
													set_container_child_insensitive, toggle);
}
	...
	if (!visible) {
		if (get_flags (self->flags, ASGTK_MYSTYLE_EDIT_NAME_VISIBLE)) {
			gtk_container_remove (GTK_CONTAINER (self),
														self->hbox1_mystyle_name);
			clear_flags (self->flags, ASGTK_MYSTYLE_EDIT_NAME_VISIBLE);
		}
	} else {
		if (!get_flags (self->flags, ASGTK_MYSTYLE_EDIT_NAME_VISIBLE)) {
			gtk_box_pack_start (GTK_BOX (self), self->hbox1_mystyle_name, TRUE,
													TRUE, 0);
...
asgtk_mystyle_edit_set_background_type (ASGtkMyStyleEdit * self, int type)
{
	...
	gtk_combo_box_set_active (GTK_COMBO_BOX (self->combo_background_type),
														-1);
	...
	if (type == 0) {
		...
		case TEXTURE_GRADIENT_BL2TR:
			back_type = ASGtkMSB_GradientRT2LB;
			break;
	...
	} else if (type >= TEXTURE_TEXTURED_START
						 && type <= TEXTURE_TEXTURED_END) {
		...
		if (type == TEXTURE_SHAPED_SCALED_PIXMAP)
			back_type =
					get_flags (self->flags,
										 ASGTK_MYSTYLE_SLICED) ? ASGtkMSB_SlicedShaped :
					ASGtkMSB_ScaledShaped;
		...
		else if (type >= TEXTURE_SCALED_TRANSPIXMAP
						 && type < TEXTURE_SCALED_TRANSPIXMAP_END)
			back_type =
					get_flags (self->flags,
										 ASGTK_MYSTYLE_SLICED) ? ASGtkMSB_SlicedPseudoTransp :
					ASGtkMSB_ScaledPseudoTransp;
...
void asgtk_mystyle_edit_set_sliced (ASGtkMyStyleEdit * self, Bool sliced)
{
...
static void
asgtk_mystyle_edit_set_style_def (MyStyleDefinition * msd,
																	ASGtkMyStyleEdit * self)
{
	ASGtkSimpleList *inherit_list =
			ASGTK_SIMPLE_LIST (self->tw_inherit_list);
	int i;
	...
	gtk_combo_box_set_active (GTK_COMBO_BOX (self->combo_shadow_type),
														AST_3DTypes);
	gtk_combo_box_set_active (GTK_COMBO_BOX (self->combo_overlay_mystyle),
														-1);
...
void
asgtk_mystyle_edit_set_mystyles_list (ASGtkMyStyleEdit * self,
																			GtkTreeModel * list)
{
	if (self) {
		...
		gtk_combo_box_set_model (GTK_COMBO_BOX (self->combo_overlay_mystyle),
														 self->mystyles_list);
	}
}
...
static void asgtk_look_edit_style_set (GtkWidget * widget,
																			 GtkStyle * prev_style);
...
GType asgtk_look_edit_get_type (void)
{
	static GType id_type = 0;
	...
	if (!id_type) {
		...
		id_type =
				g_type_register_static (GTK_TYPE_VBOX, "ASGtkLookEdit", &id_info,
																0);
	}
...
static void asgtk_look_edit_init (ASGtkLookEdit * self)
{
...
static void
asgtk_look_edit_style_set (GtkWidget * widget, GtkStyle * prev_style)
{
	/* ASGtkImageDir *id = ASGTK_IMAGE_DIR (widget); */

	GTK_WIDGET_CLASS (look_edit_parent_class)->style_set (widget,
																												prev_style);
}
...
static void
on_add_to_library_mystyle_btn_clicked (GtkButton * button,
																			 gpointer user_data)
{
	...
	if (msd) {
		...
		if (filename) {
			FreeStorageElem *fs =
					MyStyleDefinitionsList2free_storage (msd, &LookSyntax);

			if (fs) {
				WriteFreeStorageToFile (filename, "afterstep", &MyStyleSyntax,
																fs->sub, 0);
...
void
mystyle_panel_sel_handler (GObject * selection_owner, gpointer user_data)
{
...
static void build_mystyles_panel (ASGtkMyStylesPanel * panel)
{
	...
	panel->list_window =
			ASGTK_SCROLLED_WINDOW (GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC,
														 GTK_SHADOW_IN);
	...
	fprintf (stderr, "$$$$$$ panel = %p, msedit = %p\n", panel,
					 panel->mystyle_editor);
	asgtk_mystyle_edit_set_mystyles_list (ASGTK_MYSTYLE_EDIT
																				(panel->mystyle_editor),
																				ASGTK_SIMPLE_LIST (panel->list)->
																				tree_model);
	asgtk_mystyle_edit_set_name_visible (ASGTK_MYSTYLE_EDIT
																			 (panel->mystyle_editor), False);

	asgtk_simple_list_set_sel_handling (ASGTK_SIMPLE_LIST (panel->list),
																			G_OBJECT (panel->mystyle_editor),
																			mystyle_panel_sel_handler);

	gtk_container_add (GTK_CONTAINER (panel->mystyle_frame),
										 panel->mystyle_editor);
	...
	panel->list_add_btn =
			asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_ADD, "Add",
															 G_CALLBACK (on_add_mystyle_btn_clicked),
															 panel);
	panel->list_del_btn =
			asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_DELETE, "Delete",
															 G_CALLBACK (on_add_mystyle_btn_clicked),
															 panel);
	panel->list_rename_btn =
			asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_PREFERENCES,
															 "Rename",
															 G_CALLBACK (on_add_mystyle_btn_clicked),
															 panel);
	ASGTK_PACK_END;
	ASGTK_PACK_BEGIN (panel->list_hbtn_box2);
	panel->list_tolib_btn =
			asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_CONVERT,
															 "Add to Library",
															 G_CALLBACK
															 (on_add_to_library_mystyle_btn_clicked),
															 panel);
	panel->list_saveas_btn =
			asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_SAVE_AS,
															 "Save As",
															 G_CALLBACK (on_save_as_mystyle_btn_clicked),
															 panel);
	ASGTK_PACK_END;
	InitSession ();

	ASGTK_CONTAINER_CONFIG (panel->list_window, MYSTYLE_LIST_WIDTH,
													MYSTYLE_LIST_HEIGHT, 0);
...
static void
MyStyleDefs2MyStylesPanel (MyStyleDefinition * style_defs,
													 ASGtkMyStylesPanel * panel)
{
...
/*  public functions  */
GtkWidget *asgtk_look_edit_new (const char *myname,
																struct SyntaxDef *syntax)
{
	...
	self->myframes_frame =
			asgtk_collapsing_frame_new ("MyFrames - window frame config", NULL);
	self->balloons_frame = asgtk_collapsing_frame_new ("Balloons", NULL);
	self->buttons_frame =
			asgtk_collapsing_frame_new ("Titlebar Buttons", NULL);
	self->backgrounds_frame =
			asgtk_collapsing_frame_new ("Root backgrounds config", NULL);
...
asgtk_look_edit_set_configfile (ASGtkLookEdit * self, char *fulldirname)
{
	...
	if (self->configfilename == NULL && fulldirname == NULL)
		return;
	if (self->configfilename && fulldirname
			&& strcmp (self->configfilename, fulldirname) == 0)
		return;
...
void asgtk_look_edit_reload (ASGtkLookEdit * self)
{
	...
	if (self->configfilename != NULL) {
		...
		show_progress ("configuration loaded from \"%s\" ...",
									 self->configfilename);
	}
	if (self->config != NULL) {
...
int main (int argc, char *argv[])
{
	...
	asgtk_look_edit_set_configfile (ASGTK_LOOK_EDIT (look_edit),
																	PutHome
																	("~/.afterstep/looks/look.Mine"));
