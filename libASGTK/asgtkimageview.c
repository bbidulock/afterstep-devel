...
static void asgtk_image_view_style_set (GtkWidget * widget,
																				GtkStyle * prev_style);

/*  private variables  */
static GtkFrameClass *parent_class = NULL;

GType asgtk_image_view_get_type (void)
{
	...
	if (!iv_type) {
		...
		iv_type =
				g_type_register_static (GTK_TYPE_FRAME, "ASGtkImageView", &iv_info,
																0);
	}
...
static gfloat get_old_ratio (ASGtkImageView * iv)
{
	gfloat old_ratio;
...
static void setup_asgtk_image_view_layout_vert (ASGtkImageView * iv)
{
	GtkWidget *main_vbox;
	...
	gtk_box_pack_end (GTK_BOX (main_vbox), iv->details_frame, FALSE, FALSE,
										5);
														...
														ASImageListEntry * image_entry)
{
	...
	LOCAL_DEBUG_OUT (" ASGtk image view object's %p entry to %p", iv,
									 image_entry);
	unref_asimage_list_entry (iv->image_entry);
...
ASImageListEntry *asgtk_image_view_get_entry (ASGtkImageView * iv)
{
...
void
asgtk_image_view_add_detail (ASGtkImageView * iv, GtkWidget * detail,
														 int spacing)
{
	g_return_if_fail (ASGTK_IS_IMAGE_VIEW (iv));
	if (detail) {
		gtk_box_pack_start (GTK_BOX (iv->details_hbox), detail, FALSE, FALSE,
												spacing);
		gtk_box_reorder_child (GTK_BOX (iv->details_hbox), iv->details_label,
													 -1);
	}
