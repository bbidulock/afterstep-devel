...
static void asgtk_xml_opt_list_style_set (GtkWidget * widget,
																					GtkStyle * prev_style);
...
GType asgtk_xml_opt_list_get_type (void)
{
...
static void asgtk_xml_opt_list_init (ASGtkXmlOptList * self)
{
...
static void
asgtk_xml_opt_list_sel_handler (GtkTreeSelection * selection,
																gpointer user_data)
{
...
GtkWidget *asgtk_xml_opt_list_new ()
{
	...
	const char *column_names[ASGTK_XmlOptList_Cols] =
			{ "Module", "Keyword", "Id", "Value" };
	int i;
	int default_columns =
			ASGTK_XmlOptList_DefaultFlags & ASGTK_XmlOptList_Cols_All;
	...
	self->tree_model =
			GTK_TREE_MODEL (gtk_list_store_new
											(ASGTK_XmlOptList_Cols + 1, G_TYPE_STRING,
	...
	g_signal_connect (selection, "changed",
										G_CALLBACK (asgtk_xml_opt_list_sel_handler), self);
...
asgtk_xml_opt_list_set_columns (ASGtkXmlOptList * self, ASFlagType columns)
{
	...
	for (i = 0; i < ASGTK_XmlOptList_Cols; ++i) {
		...
		if (get_flags (columns, flag)) {
			gtk_tree_view_insert_column (self->tree_view,
																	 GTK_TREE_VIEW_COLUMN (self->columns[i]),
																	 i);
			set_flags (self->flags, flag);
		} else if (get_flags (self->flags, flag)) {
			gtk_tree_view_remove_column (self->tree_view,
																	 GTK_TREE_VIEW_COLUMN (self->
																												 columns[i]));
...
void asgtk_xml_opt_list_refresh (ASGtkXmlOptList * self)
{
	int items = 0;
		...
		if (xml_opt_tree_go_first (self->opt_list_context)) {
			do {
				...
				gtk_list_store_set (GTK_LIST_STORE (self->tree_model), &iter,
														ASGTK_XmlOptList_Col_Module_No,
														self->opt_list_context->current_module,
														ASGTK_XmlOptList_Col_Keyword_No,
														self->opt_list_context->current_keyword,
														ASGTK_XmlOptList_Col_Id_No,
														self->opt_list_context->current_id,
														ASGTK_XmlOptList_Col_Value_No,
														self->opt_list_context->current_cdata,
														ASGTK_ImageDir_Cols,
														self->opt_list_context->current, -1);
	...
	if (items == 0) {
		asgtk_xml_opt_list_sel_handler (gtk_tree_view_get_selection
																		(self->tree_view), self);
	}
}
...
void
asgtk_xml_opt_list_set_configfile (ASGtkXmlOptList * self,
																	 char *fulldirname)
{
	g_return_if_fail (ASGTK_IS_XML_OPT_LIST (self));
...
void
asgtk_xml_opt_list_set_sel_handler (ASGtkXmlOptList * self,
																		_ASGtkXmlOptList_sel_handler
																		sel_change_handler, gpointer user_data)
{
	g_return_if_fail (ASGTK_IS_XML_OPT_LIST (self));
