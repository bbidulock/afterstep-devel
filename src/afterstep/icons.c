...
ASIconBox *get_iconbox (int desktop)
{
	...
	if (IsValidDesk (desktop)) {
		...
		else if (Scr.icon_boxes) {
			ASHashData hdata;
			if (get_hash_item
					(Scr.icon_boxes, AS_HASHABLE (desktop),
		...
		if (ib == NULL) {
			...
			if (Scr.Look.configured_icon_areas_num == 0
					|| Scr.Look.configured_icon_areas == NULL) {
				ib->areas_num = 1;
			...
			if (get_flags (Scr.Feel.flags, StickyIcons))
				...
				if (add_hash_item (Scr.icon_boxes, AS_HASHABLE (desktop), ib) !=
						ASH_Success)
...
Bool rearrange_icon_iter_func (void *data, void *aux_data)
{
	...
	LOCAL_DEBUG_OUT
			("asw(%p)->magic(%lX)->icon_canvas(%p)->icon_title_canvas(%p)->icon_button(%p)->icon_title(%p)",
			 asw, asw->magic, asw->icon_canvas, asw->icon_title_canvas,
			 asw->icon_button, asw->icon_title);
	...
	while (rd->curr_area < rd->ib->areas_num) {
		...
		LOCAL_DEBUG_OUT ("trying area #%d : %s%s, %dx%d%+d%+d", rd->curr_area,
										 get_flags (geom->flags, XNegative) ? "XNeg" : "XPos",
										 get_flags (geom->flags, YNegative) ? "YNeg" : "YPos",
										 geom->width, geom->height, geom->x, geom->y);
		LOCAL_DEBUG_OUT ("last: %dx%d%+d%+d", rd->last_width, rd->last_height,
										 rd->last_x, rd->last_y);
		...
		if (*new_p < 0 || *new_p + *whole_p > (int)*geom_size_p) {	/* lets try and go to the next row and see if that makes a difference */
			...
			*last_p =
					get_flags (geom->flags, PNegative) ? (int)*geom_size_p - 1 : 0;
	...
	/* placing the icon : */
	LOCAL_DEBUG_OUT
			("placing an icon at %+d%+d, base %+d%+d whole %dx%d button %dx%d",
			 x, y, box_x, box_y, whole_width, whole_height, width, height);
	if (asw->icon_title_canvas && asw->icon_title_canvas != asw->icon_canvas) {
		moveresize_canvas (asw->icon_canvas,
											 box_x + x + (title_width - width) / 2, box_y + y,
											 width, height);
		moveresize_canvas (asw->icon_title_canvas, box_x + x,
											 box_y + y + height, title_width, title_height);
	} else
		moveresize_canvas (asw->icon_canvas, box_x + x, box_y + y, whole_width,
											 whole_height);
...
void rearrange_iconbox (ASIconBox * ib)
{
	...
	iterate_asbidirlist (ib->icons, rearrange_icon_iter_func, &aux_data,
											 NULL, False);
}
...
void on_icon_changed (ASWindow * asw)
{
	...
	if (asw->icon_title) {
		...
		render_astbar (asw->icon_title,
									 asw->icon_title_canvas ? asw->icon_title_canvas : asw->
									 icon_canvas);
	}
	if (asw->icon_button) {
