...
ASWharfFolder *build_wharf_folder (WharfButton * list,
																	 ASWharfButton * parent, Bool vertical);
Bool display_wharf_folder (ASWharfFolder * aswf, int left, int top,
...
int main (int argc, char **argv)
{
	...
	WharfState.root_folder =
			build_wharf_folder (Config->root_folder, NULL,
													(Config->columns > 0));
	...
	if (!display_main_folder ()) {
		show_error
				("main folder does not have any entries or has zero size. Aborting!");
		return 1;
	}
...
Bool check_style_shaped (MyStyle * style)
{
	...
	if (style->texture_type >= TEXTURE_TEXTURED_START &&
		...
		if (style->back_icon.image
				&& check_asimage_alpha (Scr.asv, style->back_icon.image)) {
...
void SetWharfLook ()
{
	...
	LOCAL_DEBUG_OUT ("Will use style \"%s\"",
									 Scr.Look.MSWindow[UNFOCUSED_TILE_STYLE]->name);
	...
	if (menu_folder_pixmap && menu_folder_pixmap->back_icon.image != NULL) {
		if (menu_folder_pixmap->back_icon.image->name == NULL) {
			release_asimage_by_name (Scr.image_manager,
															 ASXMLVAR_MenuFolderPixmap);
			store_asimage (Scr.image_manager,
										 menu_folder_pixmap->back_icon.image,
										 ASXMLVAR_MenuFolderPixmap);
			/* and increment refcount : */
			menu_folder_pixmap->back_icon.image =
					dup_asimage (menu_folder_pixmap->back_icon.image);
		}
	}
	...
	if (get_flags (Config->set_flags, WHARF_FORCE_SIZE)) {
		...
		if (Scr.Look.MSWindow[UNFOCUSED_TILE_STYLE]->back_icon.image != NULL) {
			Config->force_size.width =
					Scr.Look.MSWindow[UNFOCUSED_TILE_STYLE]->back_icon.width;
			Config->force_size.height =
					Scr.Look.MSWindow[UNFOCUSED_TILE_STYLE]->back_icon.height;
		} else {
			Config->force_size.width = 64;
			Config->force_size.height = 64;
		}
...
/* Window ID xref :                                                         */
/****************************************************************************/
Bool register_object (Window w, ASMagic * obj)
{
	if (WharfState.win2obj_xref == NULL)
		WharfState.win2obj_xref = create_ashash (0, NULL, NULL, NULL);

	return (add_hash_item (WharfState.win2obj_xref, AS_HASHABLE (w), obj) ==
					ASH_Success);
}

ASMagic *fetch_object (Window w)
{
	ASHashData hdata = { 0 };
	if (WharfState.win2obj_xref)
		if (get_hash_item
				(WharfState.win2obj_xref, AS_HASHABLE (w),
				 &hdata.vptr) != ASH_Success)
			hdata.vptr = NULL;
	return hdata.vptr;
}

void unregister_object (Window w)
{
	if (WharfState.win2obj_xref)
		remove_hash_item (WharfState.win2obj_xref, AS_HASHABLE (w), NULL,
											False);
}

Bool register_swallow_target (char *name, ASWharfButton * aswb)
{
	if (name && aswb) {
		if (WharfState.swallow_targets == NULL)
			WharfState.swallow_targets =
					create_ashash (0, casestring_hash_value, casestring_compare,
												 NULL);

		return (add_hash_item
						(WharfState.swallow_targets, AS_HASHABLE (name),
						 aswb) == ASH_Success);
	}
	return False;
}

ASWharfButton *fetch_swallow_target (char *name)
{
	ASHashData hdata = { 0 };
...
void unregister_swallow_target (char *name)
{
	if (WharfState.swallow_targets && name)
		remove_hash_item (WharfState.swallow_targets, AS_HASHABLE (name), NULL,
											False);
}
...
void process_message (send_data_type type, send_data_type * body)
{
	...
	if ((type & WINDOW_PACKET_MASK) != 0) {
		...
		if (res == WP_DataCreated || res == WP_DataChanged) {
			...
			LOCAL_DEBUG_OUT ("client deleted (%p)->window(%lX)->desk(%d)",
											 saved_wd, saved_w, saved_desk);
		}
	...
	case ClientMessage:
		{
			...
			LOCAL_DEBUG_OUT ("ClientMessage(\"%s\",data=(%lX,%lX,%lX,%lX,%lX)",
											 name, event->x.xclient.data.l[0],
											 event->x.xclient.data.l[1],
											 event->x.xclient.data.l[2],
											 event->x.xclient.data.l[3],
											 event->x.xclient.data.l[4]);
			XFree (name);
#endif
...
ASCanvas *create_wharf_folder_canvas (ASWharfFolder * aswf)
{
	...
	if (Scr.asv->visual_info.visual !=
			DefaultVisual (dpy, DefaultScreen (dpy)))
		mask |= CWBackPixel;
	w = create_visual_window (Scr.asv, Scr.Root, 0, 0, 2, 2, 0, InputOutput,
														mask, &attr);
...
ASCanvas *create_wharf_button_canvas (ASWharfButton * aswb,
																			ASCanvas * parent)
{
	static XSetWindowAttributes attr;
	Window w;
	...
	attr.background_pixel = Scr.asv->black_pixel;
	w = create_visual_window (Scr.asv, parent->w, -1, -1, 1, 1, 0,
														InputOutput, CWEventMask | CWBackPixel, &attr);
														...
														ASTBarData * bar)
{
	...
	if (get_flags (Config->ShowHints, WHARF_SHOW_HINT_Comment)
			&& wb->comment)
...
ASTBarData *build_wharf_button_tbar (ASWharfButton * aswb,
																		 WharfButton * wb)
{
	...
	int label_flip =
			get_flags (Config->flags, WHARF_FlipLabel) ? FLIP_UPSIDEDOWN : 0;
...
#define WLL_OverIcon    (0x01<<4)
	...
	LOCAL_DEBUG_OUT ("Building contents...");
	if (wb->contents) {
		char **icon = NULL;
		Bool no_icon = False;
		if (wb->selected_content >= 0
				&& wb->selected_content < wb->contents_num) {
			icon = wb->contents[wb->selected_content].icon;
			if (icon == NULL
					&& IsSwallowFunc (wb->contents[wb->selected_content].function->
														func))
				no_icon = True;
		}
		...
		if (icon) {
			register int i = -1;
			int desired_size = max (aswb->desired_width, Config->force_size.width);
			if (desired_size == 0)
				desired_size = max (aswb->desired_height, Config->force_size.height);
			if (desired_size == 0)
				desired_size = 64;
			while (icon[++i]) {				/* load image here */
				ASImage *im = load_environment_icon_any (icon[i], desired_size);
				LOCAL_DEBUG_OUT ("icon[%d] = \"%s\", loaded im = %p", i, icon[i], im);
				if (im) {
					int scale_to_width = aswb->desired_width > 0 ? aswb->desired_width : Config->force_size.width;
					int scale_to_height = aswb->desired_height > 0 ? aswb->desired_height : Config->force_size.height;
					if (scale_to_width == 0) scale_to_width = im->width;
					if (scale_to_height == 0) scale_to_height = im->height;
					LOCAL_DEBUG_OUT
							("icon = \"%s\", im_size = %dx%d, desired = %dx%d, to_scale = %dx%d",
							 icon[i], im->width, im->height, aswb->desired_width,
							 aswb->desired_height, scale_to_width, scale_to_height);
					if ( scale_to_width < im->width || scale_to_height < im->height) {
						ASImage *tmp =
								scale_asimage (Scr.asv, im, min(scale_to_width,im->width),
															 min(scale_to_height,im->height), ASA_ASImage, 100,
					...
					add_astbar_icon (bar, icon_col, icon_row, 0,
													 Config->AlignContents, im);
				}
			}
		}
	}
	...
	set_astbar_composition_method (bar, BAR_STATE_UNFOCUSED,
																 Config->CompositionMethod);

	if (!get_flags (wb->set_flags, WHARF_BUTTON_TRANSIENT)) {
		if (get_flags (Config->flags, WHARF_SHOW_LABEL) && wb->title) {
			add_astbar_label (bar, label_col, label_row, label_flip, label_align,
												2, 2, wb->title, get_flags (wb->set_flags,
																										WHARF_BUTTON_TITLE_IS_UTF8)
												? AS_Text_UTF8 : AS_Text_ASCII);
	...
	LOCAL_DEBUG_OUT ("wharf bevel is %s, value 0x%lX, wharf_no_border is %s",
									 get_flags (Config->set_flags,
															WHARF_Bevel) ? "set" : "unset",
									 Config->Bevel, get_flags (Config->flags,
																						 WHARF_NO_BORDER) ? "set" :
									 "unset");
	if (get_flags (Config->flags, WHARF_NO_BORDER)) {
		...
		set_astbar_hilite (bar, BAR_STATE_UNFOCUSED,
											 RIGHT_HILITE | BOTTOM_HILITE);
		set_astbar_hilite (bar, BAR_STATE_FOCUSED,
											 RIGHT_HILITE | BOTTOM_HILITE);
	}
	return bar;
}
...
void update_wharf_button_content_from_de (WharfButtonContent * content,
																					ASDesktopEntry * de)
{
	if (de->Icon) {
		content->icon = safecalloc (2, sizeof (char *));
		content->icon[0] = mystrdup (de->Icon);
...
WharfButton *desktop_category2wharf_folder (WharfButton * owner,
																						WharfButtonContent * content,
																						...
																						int max_depth)
{
	...
	if ((dc = name2desktop_category (category_name, &ct)) != NULL) {
		...
		entries =
				desktop_category_get_entries (ct, dc, max_depth, NULL,
																			&entries_num);
		if (entries) {
			for (i = 0; i < entries_num; ++i) {
				...
				*tail = curr;
				...
				if (de->type == ASDE_TypeDirectory) {
					curr->folder =
							desktop_category2wharf_folder (NULL, NULL, de->Name,
																						 max_depth);
				}
			}
			free (entries);
	...
	while (wb) {
		...
		if (get_flags (wb->set_flags, WHARF_BUTTON_TRANSIENT))
			...
			for (i = 0; i < wb->contents_num; ++i) {
				...
				LOCAL_DEBUG_OUT ("contents %d has function %p with func = %ld", i,
												 function, function ? function->func : -1);
				if (function) {
					int func = function->func;
					if (func == F_DesktopEntry) {
						char *de_name =
								function->text ? function->text : function->name;
						...
						char *cat_name =
								function->text ? function->text : function->name;
						desktop_category2wharf_folder (wb, &(wb->contents[i]),
																					 cat_name,
																					 (func == F_CATEGORY) ? 1 : 5);
		...
		while (wb) {
			...
			if (!get_flags (aswb->flags, ASW_SwallowTarget)) {
				if (aswb->desired_width == 0)
					aswb->desired_width =
							(Config->force_size.width ==
							 0) ? calculate_astbar_width (aswb->bar) : Config->
							force_size.width;
				if (aswb->desired_height == 0)
					aswb->desired_height =
							(Config->force_size.height ==
							 0) ? calculate_astbar_height (aswb->bar) : Config->
							force_size.height;
			} else {
				if (aswb->desired_width == 0)
			...
			if (wb->folder)
				aswb->folder =
						build_wharf_folder (wb->folder, aswb, vertical ? False : True);
...
void destroy_wharf_folder (ASWharfFolder ** paswf)
{
	...
	if (aswf->buttons) {
		...
		while (--i >= 0) {
			...
			int i;
			if (aswb->swallowed) {
				send_wm_protocol_request (aswb->swallowed->current->w,
...
void set_folder_name (ASWharfFolder * aswf, Bool withdrawn)
{
	if (withdrawn) {
		...
		set_client_names (aswf->canvas->w, withdrawn_name, withdrawn_name,
											AS_MODULE_CLASS, CLASS_WHARF_WITHDRAWN);
		free (withdrawn_name);
	} else if (aswf == WharfState.root_folder) {
		set_client_names (aswf->canvas->w, MyName, MyName, AS_MODULE_CLASS,
											CLASS_WHARF);
	} else {
...
#define WHARF_SUBFOLDER_TEXT	"Subfolder"
			folder_name =
					safemalloc (strlen (MyName) +
											sizeof (WHARF_SUBFOLDER_TEXT " - ") +
											strlen (aswf->parent->name) + 1);
			sprintf (folder_name, "%s" WHARF_SUBFOLDER_TEXT " - %s", MyName,
							 aswf->parent->name);
		} else {
			folder_name =
					safemalloc (strlen (MyName) + sizeof (WHARF_SUBFOLDER_TEXT) + 1);
			sprintf (folder_name, "%s" WHARF_SUBFOLDER_TEXT, MyName);
		}
		set_client_names (aswf->canvas->w, folder_name, folder_name,
											AS_MODULE_CLASS, CLASS_WHARF);
		free (folder_name);
	}
}

Bool update_wharf_folder_shape (ASWharfFolder * aswf)
{
#ifdef SHAPE
	int i = aswf->buttons_num;
	int set = 0;
	...
	if ((get_flags (Config->flags, WHARF_ShapeToContents)
			 && get_flags (aswf->flags, ASW_NeedsShaping))
			|| WharfState.shaped_style
			|| get_flags (aswf->flags, ASW_UseBoundary)) {
		...
		while (--i >= 0) {
			...
			LOCAL_DEBUG_OUT
					("Adding shape of the button %d (%p) with geometry %dx%d%+d%+d, and geometry inside folder %dx%d%+d%+d",
					 i, aswb, aswb->canvas->width, aswb->canvas->height,
					 aswb->canvas->root_x, aswb->canvas->root_y, aswb->folder_width,
					 aswb->folder_height, aswb->folder_x, aswb->folder_y);
			if (aswb->canvas->width == aswb->folder_width
					&& aswb->canvas->height == aswb->folder_height)
				do_combine = True;
			if (aswb->swallowed) {
				refresh_container_shape (aswb->swallowed->current);
				LOCAL_DEBUG_OUT
						("$$$$$$ name = \"%s\" current pos = %+d%+d, button = %+d%+d",
						 aswb->name, aswb->swallowed->current->root_x,
						 aswb->swallowed->current->root_y, aswb->canvas->root_x,
						 aswb->canvas->root_y);
				combine_canvas_shape_at (aswb->canvas, aswb->swallowed->current,
																 aswb->swallowed->current->root_x -
																 aswb->canvas->root_x,
																 aswb->swallowed->current->root_y -
																 aswb->canvas->root_y);
 ...
 */
			...
			if (do_combine)
				if (combine_canvas_shape_at
						(aswf->canvas, aswb->canvas, aswb->folder_x, aswb->folder_y))
					++set;
...
#if 0
			fprintf (stderr,
							 "%s: substr_boundary = %dx%d%+d%+d canvas = %dx%d%+d%+d\n",
							 __FUNCTION__, sr.width, sr.height, sr.x, sr.y,
							 aswf->canvas->width, aswf->canvas->height,
							 aswf->canvas->root_x, aswf->canvas->root_y);
			fprintf (stderr, "shape = %p, used = %d\n", aswf->canvas->shape,
							 PVECTOR_USED (aswf->canvas->shape));
#endif
			if (sr.width > 0 && sr.height > 0 && do_subtract > 0) {
				subtract_shape_rectangle (aswf->canvas->shape, &sr, 1, 0, 0,
																	aswf->canvas->width,
																	aswf->canvas->height);
			}
		}
...
Bool update_wharf_button_styles (ASWharfButton * aswb, Bool odd)
{
	MyStyle *focused_style =
			Scr.Look.MSWindow[odd ? FOCUSED_ODD_TILE_STYLE : FOCUSED_TILE_STYLE];
	MyStyle *unfocused_style =
			Scr.Look.
			MSWindow[odd ? UNFOCUSED_ODD_TILE_STYLE : UNFOCUSED_TILE_STYLE];
	...
	return set_astbar_style_ptr (aswb->bar, BAR_STATE_UNFOCUSED,
															 unfocused_style);
}
...
void update_wharf_folder_styles (ASWharfFolder * aswf, Bool force)
{
	if (aswf) {
		...
		while (--i >= 0) {
			...
			if (aswb->
					folder /*&& get_flags( aswb->folder->flags, ASW_Mapped ) */ )
...
void map_wharf_folder (ASWharfFolder * aswf, int gravity)
{
	XSizeHints shints;
	...
	if (aswf != WharfState.root_folder) {
		XSetTransientForHint (dpy, aswf->canvas->w,
													WharfState.root_folder->canvas->w);
	...
	set_client_hints (aswf->canvas->w, NULL, &shints, AS_DoesWmDeleteWindow,
										&extwm_hints);
...
#ifdef SHAPE
	if (get_flags (Config->flags, WHARF_ANIMATE)) {
		XShapeCombineRectangles (dpy, aswf->canvas->w, ShapeBounding, 0, 0,
														 &(aswf->boundary), 1, ShapeSet, Unsorted);
		aswf->animate_from_w = 1;
		aswf->animate_from_h = 1;
		set_flags (aswf->flags, ASW_AnimationPending);
	}
...
void
place_wharf_buttons (ASWharfFolder * aswf, int *total_width_return,
										 int *total_height_return)
{
	...
	*total_height_return = 0;
	...
	if (get_flags (aswf->flags, ASW_Vertical)) {
		...
		if (buttons_per_column > Scr.MyDisplayHeight / 64) {	/* too many buttons - let's add some columns, shall we */
			int avg_height = 0;
			for (i = 0; i < aswf->buttons_num; ++i)
				avg_height +=
						(!get_flags (aswf->buttons[0].flags, ASW_MaxSwallow)) ? aswf->
						buttons[0].desired_height : 64;
			...
			if (buttons_per_column > Scr.MyDisplayHeight / avg_height) {
				...
				columns =
						(aswf->buttons_num + buttons_per_column -
						 1) / buttons_per_column;
			}
		}
		for (i = 0; i < aswf->buttons_num; ++i) {
			ASWharfButton *aswb =
			...
			if (bc == 0) {
				...
				max_height = 0;
				while (--k >= i) {
					register ASWharfButton *aswb =
							&(aswf->buttons[reverse_order >= 0 ? reverse_order - k : k]);
					if (max_width < aswb->desired_width)
						max_width = aswb->desired_width;
					if (max_height < aswb->desired_height
							&& !get_flags (aswb->flags, ASW_MaxSwallow))
			...
			height = (get_flags (aswb->flags, ASW_MaxSwallow)
								|| fit_contents) ? aswb->desired_height : max_height;
			if (get_flags (Config->flags, WHARF_ShapeToContents)) {
			...
			moveresize_canvas (aswb->canvas, aswb->folder_x, aswb->folder_y,
												 aswb->folder_width, aswb->folder_height);
		...
		if (buttons_per_row > Scr.MyDisplayWidth / 64) {	/* too many buttons - let's add some columns, shall we */
			int avg_width = 0;
			for (i = 0; i < aswf->buttons_num; ++i)
		...
		for (i = 0; i < aswf->buttons_num; ++i) {
			...
			width = (get_flags (aswb->flags, ASW_MaxSwallow)
							 || fit_contents) ? aswb->desired_width : max_width;
			if (get_flags (Config->flags, WHARF_ShapeToContents)) {
	...
	LOCAL_DEBUG_OUT ("total_size_return(%dx%d)", *total_width_return,
									 *total_height_return);
...
void
clamp_animation_rect (ASWharfFolder * aswf, int from_width,
											int from_height, int to_width, int to_height,
											XRectangle * rect, Bool reverse)
{
	if (get_flags (aswf->flags, ASW_Vertical)) {
		Bool down = (aswf->gravity == SouthWestGravity
								 || aswf->gravity == SouthEastGravity);
		...
		Bool right = (aswf->gravity == NorthEastGravity
									|| aswf->gravity == SouthEastGravity);
		if (reverse)
			right = !right;
		if (right)
...
void
animate_wharf_loop (ASWharfFolder * aswf, int from_width, int from_height,
										int to_width, int to_height, Bool reverse)
{
	int i, steps;
	XRectangle rect;
	...
	steps =
			get_flags (Config->set_flags,
								 WHARF_ANIMATE_STEPS) ? Config->animate_steps : 12;
	LOCAL_DEBUG_OUT ("steps = %d", steps);
	for (i = 0; i < steps; ++i) {
		rect.x = rect.y = 0;
		rect.width =
				get_flags (aswf->flags,
									 ASW_Vertical) ? to_width : from_width + ((to_width -
																														 from_width) /
																														steps) * (i +
																																			1);
		rect.height =
				get_flags (aswf->flags,
									 ASW_Vertical) ? from_height + ((to_height -
																									 from_height) / steps) *
				(i + 1) : to_height;

		clamp_animation_rect (aswf, from_width, from_height, to_width,
													to_height, &rect, reverse);
	...
	rect.width = to_width;
	rect.height = to_height;
	clamp_animation_rect (aswf, from_width, from_height, to_width, to_height,
												&rect, reverse);
...
void
animate_wharf (ASWharfFolder * aswf, int *new_width_return,
							 int *new_height_return)
{
	...
	if (aswf->animation_dir < 0) {
		if (get_flags (aswf->flags, ASW_Vertical)) {
			if (aswf->animation_steps <= 1)
				...
				new_height =
						(new_height * (aswf->animation_steps - 1)) /
			...
			if (aswf->animation_steps <= 1)
				...
				new_width =
						(new_width * (aswf->animation_steps - 1)) /
						aswf->animation_steps;
		...
		if (get_flags (aswf->flags, ASW_Vertical)) {
			if (aswf->animation_steps <= 1)
				...
				new_height =
						aswf->canvas->height +
						((aswf->total_height -
							aswf->canvas->height) / aswf->animation_steps);
				if (new_height == aswf->canvas->height
						&& new_height < aswf->total_height)
					++new_height;
			}
			...
			if (aswf->animation_steps <= 1)
				...
				new_width =
						aswf->canvas->width +
						((aswf->total_width -
							aswf->canvas->width) / aswf->animation_steps);
				if (new_width == aswf->canvas->width
						&& new_width < aswf->total_width)
					++new_width;
			}
...
Bool
display_wharf_folder (ASWharfFolder * aswf, int left, int top, int right,
											int bottom)
{
	...
	Bool reverse = False;
	if (AS_ASSERT (aswf) ||
			(get_flags (aswf->flags, ASW_Mapped)
			 && !get_flags (aswf->flags, ASW_Withdrawn)))
		return False;

	if (MyArgs.gravity == SouthWestGravity
			|| MyArgs.gravity == SouthEastGravity)
		south = True;
	if (MyArgs.gravity == NorthEastGravity
			|| MyArgs.gravity == SouthEastGravity)
		east = True;
	...
	if (left < 0)
		...
		east = !(Scr.MyDisplayWidth > right
						 && (Scr.MyDisplayWidth - right > total_width
								 || left < Scr.MyDisplayWidth - right));
	...
	if (south && top < total_height)
		south = !(Scr.MyDisplayHeight > bottom
							&& (Scr.MyDisplayHeight - bottom > total_height
									|| top < Scr.MyDisplayHeight - bottom));
	...
	if (height == 0)
		height = total_height;
	LOCAL_DEBUG_OUT ("animation_steps(%d)->size(%dx%d)->total_size(%dx%d)",
									 aswf->animation_steps, width, height, total_width,
									 total_height);
	...
	if (get_flags (aswf->flags, ASW_Vertical)) {
		...
		if (left != right)
			x += east ? -Config->OrthogonalFolderOffset : Config->
		...
		if (top != bottom)
			y += south ? -Config->OrthogonalFolderOffset : Config->
					OrthogonalFolderOffset;
	}
	LOCAL_DEBUG_OUT
			("calculated pos(%+d%+d), east(%d), south(%d), total_size(%dx%d)", x,
			 y, east, south, total_width, total_height);
	if (east) {
	...
	LOCAL_DEBUG_OUT ("flags 0x%lX, reverse_order = %d", aswf->flags,
									 get_flags (aswf->flags,
															ASW_ReverseOrder) ? aswf->buttons_num -
									 1 : -1);
	...
	if (get_flags (Config->flags, WHARF_ANIMATE)) {
		...
		aswf->animate_from_w =
				get_flags (aswf->flags, ASW_Vertical) ? aswf->canvas->width : 0;
		aswf->animate_from_h =
				get_flags (aswf->flags, ASW_Vertical) ? 0 : aswf->canvas->height;
 ...
 */
...
#ifdef SHAPE
		LOCAL_DEBUG_OUT ("boundary pos(%dx%d%+d%+d) shaping window %lX",
										 aswf->boundary.width, aswf->boundary.height,
										 aswf->boundary.x, aswf->boundary.y, aswf->canvas->w);
	...
	map_wharf_folder (aswf,
										east ? (south ? SouthEastGravity : NorthEastGravity)
	...
	if (aswf->canvas->width == width && aswf->canvas->height == height
			&& get_flags (Config->flags, WHARF_ANIMATE)) {
		animate_wharf_loop (aswf, aswf->animate_from_w, aswf->animate_from_h,
												aswf->animate_to_w, aswf->animate_to_h, reverse);
...
Bool display_main_folder ()
{
	...
	return display_wharf_folder (WharfState.root_folder, left, top, left,
															 top);
}
	...
	if (aswf) {
		...
		while (--i >= 0) {
			if (get_flags (aswf->buttons[i].flags, ASW_SwallowTarget) &&
				...
				int k;
				for (k = 0; k < 5; ++k)
					if (aswf->buttons[i].fdata[k]
							&& IsSwallowFunc (aswf->buttons[i].fdata[k]->func)) {
...
void
grab_swallowed_canvas_btn (ASCanvas * canvas, int button,
													 int non_lock_mods)
{
	register int i = 0;
...
void
grab_swallowed_canvas_btns (ASCanvas * canvas, ASWharfButton * aswb,
														Bool withdraw_btn)
{
	if (aswb->folder != NULL)
...
void update_wharf_folder_size (ASWharfFolder * aswf)
{
...
void send_swallowed_configure_notify (ASWharfButton * aswb)
{
...
int make_swallow_pad (Bool pad_before, Bool pad_after, int cell_size,
											int tile_size)
{
	if (!pad_before)
		return 0;
...
void check_swallow_window (ASWindowData * wd)
{
	...
	if (wd == NULL || !get_flags (wd->state_flags, AS_Mapped))
		return;
	LOCAL_DEBUG_OUT
			("name(\"%s\")->icon_name(\"%s\")->res_class(\"%s\")->res_name(\"%s\")",
		...
		if ((aswb = fetch_swallow_target (wd->icon_name)) == NULL)
			if ((aswb = fetch_swallow_target (wd->res_class)) == NULL)
				if ((aswb = fetch_swallow_target (wd->res_name)) == NULL)
					return;
	LOCAL_DEBUG_OUT ("swallow target is %p, swallowed = %p", aswb,
									 aswb->swallowed);
	aswf = aswb->parent;
	...
	while (w == Scr.Root && ++try_num < 10) {	/* we should wait for AfterSTep to complete AddWindow protocol */
		/* do the actuall swallowing here : */
		ungrab_server ();
		sleep_a_millisec (200 * try_num);
		grab_server ();
		w = get_parent_window (wd->client);
		LOCAL_DEBUG_OUT ("attempt %d:first parent %lX, root %lX", try_num, w,
										 Scr.Root);
	}
	if (w == Scr.Root) {
		ungrab_server ();
	...
	LOCAL_DEBUG_OUT ("second parent %lX, root %lX, service_win = %lX", w,
									 Scr.Root, Scr.ServiceWin);
	...
	XReparentWindow (dpy, wd->client, aswb->canvas->w,
									 (aswb->canvas->width - nc->width) / 2,
									 (aswb->canvas->height - nc->height) / 2);
	register_object (wd->client, (ASMagic *) aswb);
	XSelectInput (dpy, wd->client,
	...
	if ((wm_normal_hints = XAllocSizeHints ()) != NULL) {
		long supplied;
		if (XGetWMNormalHints (dpy, wd->client, wm_normal_hints, &supplied) ==
				0) {
	...
	if (get_flags (wd->client_icon_flags, AS_ClientIcon)
			&& !get_flags (wd->client_icon_flags, AS_ClientIconPixmap))
		icon_w = wd->icon;
	...
	if (icon_w != None) {
		...
		XReparentWindow (dpy, icon_w, aswb->canvas->w,
										 (aswb->canvas->width - ic->width) / 2,
										 (aswb->canvas->height - ic->height) / 2);
	...
	LOCAL_DEBUG_OUT ("client(%lX)->icon(%lX)->current(%lX)", wd->client,
									 icon_w, aswb->swallowed->current->w);

	if (get_flags (aswb->flags, ASW_MaxSwallow) ||
			(Config->force_size.width == 0
			 && !get_flags (aswb->flags, ASW_FixedWidth)))
		aswb->desired_width = aswb->swallowed->current->width;
	if (get_flags (aswb->flags, ASW_MaxSwallow) ||
			(Config->force_size.height == 0
			 && !get_flags (aswb->flags, ASW_FixedHeight)))
		aswb->desired_height = aswb->swallowed->current->height;

	if (wm_normal_hints) {
		LOCAL_DEBUG_OUT ("wm_normal_hints->flags = 0x%lX",
	...
	LOCAL_DEBUG_OUT
			("original %dx%d, user %dx%d, final %dx%d, desired %dx%d",
			 aswb->swallowed->current->width, aswb->swallowed->current->height,
			 user_set_width, user_set_height, swidth, sheight,
			 aswb->desired_width, aswb->desired_height);

	moveresize_canvas (aswb->swallowed->current,
										 make_swallow_pad (get_flags
																			 (Config->AlignContents, PAD_LEFT),
																			 get_flags (Config->AlignContents,
																									PAD_RIGHT),
																			 aswb->canvas->width, swidth),
										 make_swallow_pad (get_flags
																			 (Config->AlignContents, PAD_TOP),
																			 get_flags (Config->AlignContents,
																									PAD_BOTTOM),
	...
	if (!get_flags (wd->client_icon_flags, AS_ClientIcon)) {	/* workaround for broken wmdock apps that draw into icon, that is a child of a client,
		...
		if (wm_hints) {
			LOCAL_DEBUG_OUT
					("wmfire workaround step2: icon_window_hint = %ld, icon_window = %lX",
					 get_flags (wm_hints->flags, IconWindowHint),
					 wm_hints->icon_window);
			if (get_flags (wm_hints->flags, IconWindowHint)
					&& wm_hints->icon_window != None) {
				LOCAL_DEBUG_OUT ("wmfire workaround step3: resizing to %dx%d",
												 swidth, sheight);
...
void on_wharf_button_confreq (ASWharfButton * aswb, ASEvent * event)
{
	if (aswb && aswb->swallowed) {
		...
		xwcm =
				CWX | CWY | CWWidth | CWHeight | (cre->value_mask & CWBorderWidth);
		...
		LOCAL_DEBUG_OUT
				("Configuring swallowed window %lX to req(%dx%d%+d%+d bw = %d)- actual(%dx%d%+d%+d), (flags=%lX)",
				 (unsigned long)aswb->swallowed->current->w, cre->width,
				 cre->height, cre->x, cre->y, cre->border_width, xwc.width,
				 xwc.height, xwc.x, xwc.y, xwcm);
		XConfigureWindow (dpy, aswb->swallowed->current->w, xwcm, &xwc);
		return;
	}
}

Bool on_wharf_button_moveresize (ASWharfButton * aswb, ASEvent * event)
{
	...
	if (aswb->swallowed) {
		...
		if (get_flags (changes, CANVAS_RESIZED)) {
			int swidth =
					min (aswb->desired_width, aswb->swallowed->current->width);
			int sheight =
					min (aswb->desired_height, aswb->swallowed->current->height);
	...
	if (is_canvas_dirty (aswb->canvas) || swallowed_changes != 0) {
		update_canvas_display (aswb->canvas);
#ifdef SHAPE
		if (get_flags (aswb->canvas->state, CANVAS_SHAPE_SET)
				&& aswb->swallowed) {
			update_wharf_folder_shape (aswb->parent);
		}
#endif
...
void set_wharf_clip_area (ASWharfFolder * aswf, int x, int y)
{
	if (aswf != NULL) {
		...
		LOCAL_DEBUG_OUT ("target is normal (%dx%d%+d%+d)", target_area->width,
										 target_area->height, target_area->x, target_area->y);
...
void
set_withdrawn_clip_area (ASWharfFolder * aswf, int x, int y,
												 unsigned int w, unsigned int h)
{
	if (aswf != NULL) {
		XRectangle *target_area;
		...
		LOCAL_DEBUG_OUT ("target is withdrawn (%dx%d%+d%+d)",
...
Bool render_wharf_button (ASWharfButton * aswb)
{
	...
	Bool withdrawn_root = (aswf == WharfState.root_folder
												 && get_flags (aswf->flags, ASW_Withdrawn));
	if (withdrawn_root) {
	...
	LOCAL_DEBUG_OUT ("rendering button %p for folder %p. Root Image = %p",
									 aswb, aswf, Scr.RootImage);
	if (get_flags (aswb->bar->state, BAR_FLAGS_CROP_BACK))
		result =
				render_astbar_cached_back (aswb->bar, aswb->canvas, NULL,
																	 aswf->canvas);
	else
...
void
add_wharf_folder_to_area (ASWharfFolder * aswf, int *from_x, int *from_y,
													int *to_x, int *to_y)
{
	if (aswf != NULL && get_flags (aswf->flags, ASW_Mapped)) {
		...
		if (aswf->gravity == NorthEastGravity
				|| aswf->gravity == SouthEastGravity)
			x1 = x1 + aswf->canvas->width - x2;
		if (aswf->gravity == SouthWestGravity
				|| aswf->gravity == SouthEastGravity)
			y1 = y1 + aswf->canvas->height - y2;
		x2 += x1;
		y2 += y1;
		LOCAL_DEBUG_OUT
				("folder: from %+d%+d to %+d%+d, all: from %+d%+d to %+d%+d", x1,
				 y1, x2, y2, *from_x, *from_y, *to_x, *to_y);
		if (x2 > 0 && x1 < (int)Scr.MyDisplayWidth && y2 > 0
				&& y1 < (int)Scr.MyDisplayHeight) {
			...
			LOCAL_DEBUG_OUT
					("CLAMPED: folder: from %+d%+d to %+d%+d, all: from %+d%+d to %+d%+d",
...
void do_wharf_animate_iter (void *vdata)
{
	...
	if (aswf != NULL && aswf->animation_steps > 0) {
		...
		if (new_width == 0 || new_height == 0	/*||
			...
			LOCAL_DEBUG_OUT ("resizing folder from %dx%d to %dx%d",
											 aswf->canvas->width, aswf->canvas->height,
			...
			if (get_flags (Config->set_flags, WHARF_ANIMATE_DELAY)
					&& Config->animate_delay > 0) {
				timer_new (Config->animate_delay + 1, do_wharf_animate_iter,
									 vdata);
			} else
...
void on_wharf_moveresize (ASEvent * event)
{
	...
	if (obj->magic == MAGIC_WHARF_BUTTON) {
		...
		while (ASCheckTypedWindowEvent
					 (aswb->parent->canvas->w, ConfigureNotify, &(parent_event.x))) {
		...
		LOCAL_DEBUG_OUT ("Handling folder resize for folder %p, mapped = %lX",
										 aswf, get_flags (aswf->flags, ASW_Mapped));
		if (aswf->animation_steps == 0 && get_flags (aswf->flags, ASW_Mapped)
			...
			withdrawn = (aswf->canvas->width == 1 || aswf->canvas->height == 1 ||
									 (aswf->canvas->root_x == -10000
										&& aswf->canvas->root_y == -10000));

#ifdef SHAPE
			if (get_flags (changes, CANVAS_RESIZED)
					&& get_flags (aswf->flags, ASW_AnimationPending))
				XShapeCombineRectangles (dpy, aswf->canvas->w, ShapeBounding, 0, 0,
																 &(aswf->boundary), 1, ShapeSet, Unsorted);
#endif
			...
			if (!withdrawn) {
				if (!get_flags (aswf->flags, ASW_Withdrawn)) {
					set_wharf_clip_area (aswf, aswf->canvas->root_x,
															 aswf->canvas->root_y);
					while (--i >= 0)
...
#if 1
			if (get_flags (changes, CANVAS_RESIZED)) {
				LOCAL_DEBUG_OUT ("AnimationPending ? = %lX",
												 get_flags (aswf->flags, ASW_AnimationPending));
				if (get_flags (aswf->flags, ASW_AnimationPending)) {
					LOCAL_DEBUG_OUT ("animate from = %dx%d, to = %dx%d",
													 aswf->animate_from_w, aswf->animate_from_h,
													 aswf->animate_to_w, aswf->animate_to_h);
					animate_wharf_loop (aswf, aswf->animate_from_w,
															aswf->animate_from_h, aswf->animate_to_w,
															aswf->animate_to_h, get_flags (aswf->flags,
																														 ASW_ReverseAnimation));
					clear_flags (aswf->flags,
											 ASW_UseBoundary | ASW_AnimationPending |
											 ASW_ReverseAnimation);
				} else if (!withdrawn) {
...
{
	if (WharfState.pressed_button && WharfState.pressed_button != aswb) {
		set_astbar_pressed (WharfState.pressed_button->bar,
												WharfState.pressed_button->canvas, False);
		WharfState.pressed_button = NULL;
	}
	if (aswb && WharfState.pressed_button != aswb) {
		...
		if (pressed->folder && !(button > 0 && pressed->fdata[button])) {
			...
			if (get_flags (pressed->folder->flags, ASW_Mapped))
				...
				display_wharf_folder (pressed->folder, pressed->canvas->root_x,
															...
															pressed->canvas->root_x +
															pressed->canvas->width,
															pressed->canvas->root_y +
															pressed->canvas->height);
		} else if (pressed->fdata[button]) {
#if defined(LOCAL_DEBUG) && !defined(NO_DEBUG_OUTPUT)
			print_func_data (__FILE__, __FUNCTION__, __LINE__,
											 pressed->fdata[button]);
#endif
			if (button > 0 || !get_flags (pressed->flags, ASW_SwallowTarget) || pressed->swallowed == NULL) {	/* send command to the AS-proper : */
	...
	if (obj->magic == MAGIC_WHARF_BUTTON) {
		...
		if (event->x.xbutton.button == Button3
					...
					if (get_flags (WharfState.root_folder->flags, ASW_Withdrawn)) {
						...
						if (get_flags (Config->flags, WHARF_ANIMATE)) {
							...
							animate_wharf_loop (aswf, aswf->canvas->width,
																	aswf->canvas->height, wwidth, wheight,
																	reverse);
			...
			if (event->x.xbutton.button == Button1
					|| aswb->fdata[event->x.xbutton.button - Button1] == NULL) {
				if (check_app_click (aswb, &(event->x.xbutton))) {
					event->x.xbutton.window = aswb->swallowed->current->w;
					XSendEvent (dpy, aswb->swallowed->current->w, False,
											ButtonPressMask, &(event->x));
					return;
				}
			}
		}
